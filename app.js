...
// Picker modal: lets the person search and choose one of `candidates`
// ({id, name} objects). Resolves the chosen id, or null if cancelled.
function showPicker({ title, instructions, candidates }) {
  return new Promise((resolve) => {
    const overlay = $("modalOverlay");
    const titleEl = $("modalTitle");
    const bodyEl = $("modalBody");
    ...
    bodyEl.innerHTML = `<p style="margin:0 0 10px 0;">${esc(instructions)}</p><input type="text" id="pickerSearch" placeholder="${esc(t("searchPlaceholder"))}"><div id="pickerList" class="picker-list" role="listbox"></div>`;
    ...
    const renderList = (query) => {
      ...
      if (!filtered.length) {
        list.innerHTML = `<div class="picker-empty">${esc(t("noSearchResults"))}</div>`;
        return;
      }
      list.innerHTML = filtered.map(c => `<button type="button" class="picker-item" role="option" data-id="${c.id}">${esc(c.name)}</button>`).join("");
      list.querySelectorAll(".picker-item").forEach(btn => {
        btn.addEventListener("click", () => { cleanup(); resolve(Number(btn.dataset.id)); });
      });
    };
    renderList("");
...
function updateSelected() {
  ...
  if (selectedId === null) {
    box.innerHTML = `<strong>${esc(t("none"))}</strong>${esc(t("clickPerson"))}`;
...
function refreshActionButtons(p) {
  ...
  $("edit").disabled = !p;
  $("delete").disabled = !p;
  $("attachPerson").disabled = !p || people.length < 2;
  $("detachPerson").disabled = !p || p.parentId === null;
}
...
// Reattaches the selected person (and their whole branch) under a
// different parent, which is how two separate trees get merged into one.
// Candidates exclude the person themself and all of their own descendants,
// so a cycle can never be picked in the first place; the extra check below
// is a defensive backstop only.
async function attachSelected() {
  if (selectedId === null) { alert(t("selectFirst")); return; }
  const p = get(selectedId); if (!p) return;
  const excluded = new Set([p.id, ...collectDesc(p.id)]);
  const candidates = people.filter(x => !excluded.has(x.id));
  if (!candidates.length) { alert(t("noCandidates")); return; }
  const targetId = await showPicker({
    title: `${t("attach")}: ${p.name}`,
    instructions: t("attachInstructions"),
    candidates
  });
  if (targetId === null) return;
  if (targetId === p.id || collectDesc(p.id).has(targetId)) { alert(t("cycleBlocked")); return; }
  p.parentId = targetId;
  selectedId = p.id;
  saveState(); render(); toast(t("linked"));
}

// Detaches the selected person from their current parent, turning them
// (and their descendants, who are untouched) into the root of their own
// independent tree. The inverse of attachSelected.
async function detachSelected() {
  if (selectedId === null) { alert(t("selectFirst")); return; }
  const p = get(selectedId); if (!p) return;
  if (p.parentId === null) return; // already a root
  const msg = t("detachConfirm").replace("{name}", p.name);
  const confirmed = await showModal({ title: t("detach"), body: msg, isPrompt: false });
  if (!confirmed) return;
  p.parentId = null;
  saveState(); render(); toast(t("detached"));
}
...
function updateLanguage() {
  const map = {
    ...
    selectedSection:"selectedSection",addChild:"addChild",edit:"edit",attachPerson:"attach",detachPerson:"detach",delete:"delete",designSection:"designSection",
...
$("attachPerson").addEventListener("click", attachSelected);
$("detachPerson").addEventListener("click", detachSelected);
